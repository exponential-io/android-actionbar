...
Adding menu items to an `ActionBar` created via a `Toolbar` is the same as how you would add items
to the default `ActionBar`.
...
There are 3 key steps in defining `ActionBar` actions:
...
3. Handle click events for the item in the Activity's `onOptionsItemSelected` method

In this example, we will add an action that displays a `Toast` message.

- Add the search icon image asset from the Android icon collection. Use HOLO_DARK theme icons.
